...
@Pipe({
    name: 'ngxDynamicOrder',
    standalone: true,
    pure: false,
})
export class NgxDynamicOrder implements PipeTransform {
    /**
    ...
    static caseInsensitiveSort(a: any, b: any) {
        if (NgxDynamicOrder.isString(a) && NgxDynamicOrder.isString(b)) {
            return a.localeCompare(b);
        }
        return NgxDynamicOrder.defaultCompare(a, b);
    }
    ...
    private sortArray<Type>(
        ...
        if (isDeepLink) {
            expression = NgxDynamicOrder.parseExpression(expression);
        }
        ...
        if (comparator && typeof comparator === "function") {
            ...
            compareFn = isCaseInsensitive
                ? NgxDynamicOrder.caseInsensitiveSort
                : NgxDynamicOrder.defaultCompare;
        }
        ...
        const sortedArray: any[] = array.sort((a: any, b: any): number => {
            ...
            return compareFn(
                NgxDynamicOrder.getValue(a, expression),
                NgxDynamicOrder.getValue(b, expression)
            );
        });
    ...
    private transformObject(
        ...
        const parsedExpression = NgxDynamicOrder.parseExpression(expression);
        let lastPredicate: any = parsedExpression.pop();
        let oldValue = NgxDynamicOrder.getValue(value, parsedExpression);
        ...
        if (!Array.isArray(oldValue)) {
            ...
            lastPredicate = null;
            oldValue = NgxDynamicOrder.getValue(value, parsedExpression);
        }
